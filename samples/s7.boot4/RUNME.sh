...
#
# The program puts a single message to the DEV.QUEUE.1 queue and then tries to
# retrieve it. It uses the JmsClient classes introduced in Spring Framework 7.
#
...
ps -ef|grep gradle | grep sample6.Application | awk '{print $2}' | xargs kill -9 >/dev/null 2>&1
...
./gradlew -p samples/s7.boot4 bootRun
